...
scene.fog = new THREE.FogExp2(colors.dayBg, 0.015); 
...
window.addEventListener("click", () => {
    ...
    if (butterflyRig) {
        ...
        if (intersectsButterfly.length > 0) {
            showWiki("butterfly");
            return;
        }
    }
...
btn.addEventListener("click", () =>
    applyLighting(MODE === "day" ? "night" : "day")
);
applyLighting("day");

const WORLD_RADIUS = 250; // batas
...
// Fungsi untuk membuat lantai bergelombang (AI)
...
for (let i = 0; i < particleCount * 3; i++) {
    ...
    pPos[i + 1] = Math.random() * MAX_Y_SURFACE; 
...
function setKey(e, down) {
    ...
    if (e.code === "ShiftLeft") keys.shift = down;
}
...
function animate() {
    ...
    godRays.forEach((ray, i) => {
        ray.mesh.rotation.z =
            ray.mesh.userData.initialRotZ +
            Math.sin(time * 0.5 * ray.speed + i) * 0.02;
        ray.mesh.rotation.x =
            ray.mesh.userData.initialRotX +
            Math.cos(time * 0.3 * ray.speed + i) * 0.02;

        ray.mesh.material.opacity =
            ray.baseOpacity + Math.sin(time * 1.5 * ray.speed) * 0.03;
    });

    if (whaleMixer) whaleMixer.update(dt);
    if (whaleRig) {
        whaleState.pos.addScaledVector(
            whaleState.vel.clone().setY(0),
            whaleState.speed * dt
        );
        ...
        if (
            whaleState.pos.y > whaleState.yMax ||
            whaleState.pos.y < whaleState.yMin
        )
            whaleState.vel.y *= -1;
    ...
    if (mantaRig) {
        mantaState.pos.addScaledVector(
            mantaState.vel.clone().setY(0),
            mantaState.speed * dt
        );
        ...
        if (
            mantaState.pos.y > mantaState.yMax ||
            mantaState.pos.y < mantaState.yMin
        )
            mantaState.vel.y *= -1;
    ...
    if (nemoRig) {
        nemoState.pos.addScaledVector(
            nemoState.vel.clone().setY(0),
            nemoState.speed * dt
        );
        ...
        if (nemoState.pos.y > nemoState.yMax || nemoState.pos.y < nemoState.yMin)
            nemoState.vel.y *= -1;
    ...
    if (parrotRig) {
        parrotState.pos.addScaledVector(
            parrotState.vel.clone().setY(0),
            parrotState.speed * dt
        );
        ...
        if (
            parrotState.pos.y > parrotState.yMax ||
            parrotState.pos.y < parrotState.yMin
        )
            parrotState.vel.y *= -1;
    ...
    if (butterflyRig) {
        butterflyState.pos.addScaledVector(
            butterflyState.vel.clone().setY(0),
            butterflyState.speed * dt
        );
        butterflyState.pos.y += butterflyState.vel.y * dt;
        if (Math.abs(butterflyState.pos.x) > WORLD_RADIUS)
            butterflyState.vel.x *= -1;
        if (Math.abs(butterflyState.pos.z) > WORLD_RADIUS)
            butterflyState.vel.z *= -1;
        if (
            butterflyState.pos.y > butterflyState.yMax ||
            butterflyState.pos.y < butterflyState.yMin
        )
            butterflyState.vel.y *= -1;
    ...
    if (controls.isLocked) {
        ...
        if (
            keys.space &&
            !hitObstacle(camera.position.x, nextY_up, camera.position.z)
        )
            camera.position.y = nextY_up;
        if (
            keys.shift &&
            !hitObstacle(camera.position.x, nextY_down, camera.position.z)
        )
        ...
        camera.position.y = THREE.MathUtils.clamp(
            ...
            MIN_Y,
            MAX_Y_SURFACE
        ); // Batasi kamera tidak lewat permukaan
